...
if(strlen($src) && file_exists( $src ) ) {
  ...
  $origin_y=0;
	if( $zoom_crop ) {
        ...
        imagecopyresampled( $canvas, $image, $origin_x, $origin_y, 0, 0, $new_width, $new_height, $width, $height );
...
function show_image ($mime_type, $image_resized, $quality, $cache_dir) {
	...
	$d->reset();
	$sql_dongdau = "select thumb,photo from #_background where type='dongdau' limit 0,1";
	$d->query($sql_dongdau);
